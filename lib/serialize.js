...
async function serialize(msg, client) {
  ...
  if (msg.type === "listResponseMessage") {
    const responseMessage = msg.message[msg.type];

    if (responseMessage && responseMessage.singleSelectReply) {
      // Extract the selectedRowId for the list response
      msg.body = responseMessage.singleSelectReply.selectedRowId;

      // Log the extracted message body in the desired format
      console.log(`Extracted message body: ${msg.body}`);
      console.log(`[MESSAGE]: ${msg.body}`);
      console.log(`Handling list response:`, responseMessage); // Log the entire response for further debugging
    } else {
      console.error("List response structure is not as expected:", responseMessage);
      msg.body = false;
    }
  } else if (msg.type === "buttonsResponseMessage") {
    // Handling button response messages
    msg.body = msg.message[msg.type].selectedButtonId;

    // Log for buttons response
    console.log(`Extracted message body: ${msg.body}`);
    console.log(`[MESSAGE]: ${msg.body}`);
  } else if (msg.type === "templateButtonReplyMessage") {
    // Handling template button reply messages
    msg.body = msg.message[msg.type].selectedId;

    // Log for template button response
    console.log(`Extracted message body: ${msg.body}`);
    console.log(`[MESSAGE]: ${msg.body}`);
  } else if (msg.type === "interactiveResponseMessage") {
    // Extracting from interactiveResponseMessage
    msg.body = JSON.parse(msg.message[msg.type].nativeFlowResponseMessage.paramsJson).id;

    // Log for interactive response
    console.log(`Extracted message body: ${msg.body}`);
    console.log(`[MESSAGE]: ${msg.body}`);
  } else {
    ...
    msg.body =
      ...
      false; // Fallback if no valid body is found
  }
} catch (error) {
  console.error("Error in extracting message body:", error);
  msg.body = false; // Set body to false in case of an error
}

// Log the final extracted body for verification
console.log("Final extracted message body:", msg.body);
