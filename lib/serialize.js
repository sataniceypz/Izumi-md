...
async function serialize(msg, client) {
 ...
 try {
  if (msg.type === "listResponseMessage") {
    console.log("Handling list response:", msg.message[msg.type]);
    msg.body = msg.message[msg.type]?.singleSelectReply?.selectedRowId || false; // Get selected row ID
  } else if (msg.type === "buttonsResponseMessage") {
    console.log("Handling button response:", msg.message[msg.type]);
    msg.body = msg.message[msg.type]?.selectedButtonId || false; // Get selected button ID
  } else if (msg.type === "templateButtonReplyMessage") {
    console.log("Handling template button response:", msg.message[msg.type]);
    msg.body = msg.message[msg.type]?.selectedId || false; // Get selected ID from template buttons
  } else if (msg.type === "interactiveResponseMessage") {
    console.log("Handling interactive response:", msg.message[msg.type]);
    msg.body = msg.message[msg.type]?.singleSelectReply?.selectedRowId || false; // Check for interactive message
  } else {
    msg.body =
      msg.message.conversation || // Fallback to conversation text
      msg.message[msg.type]?.text || // Text for specific message types
      msg.message[msg.type]?.caption || // Caption for specific message types
      false; // Fallback if no valid body is found
  }

  // Log the result to check what msg.body contains
