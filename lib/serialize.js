...
async function serialize(msg, client) {
  ...
  console.log("Extracted msg.body:", msg.body);

  // Check if we have a valid button ID
  if (msg.body) {
    // Handle the response based on the button ID
    switch (msg.body) {
      case 'AnimeImage':
        await sendAnimeImageResponse(message);
        break;
      case 'AnimeVideo':
        await sendAnimeVideoResponse(message);
        break;
      // Add additional cases for other buttons as necessary
      default:
        console.log("No handler for button ID:", msg.body);
        await message.reply("Sorry, I don't recognize that command.");
    }
  } else {
    console.log("No button ID found in msg.body.");
  }
} catch (error) {
  console.error("Error in processing interactive message:", error);
