...
try {
  // Start extracting message body based on type
  if (msg.type === "listResponseMessage") {
    // For list responses, extract the selected row ID from the paramsJson
    const params = JSON.parse(msg.message[msg.type].nativeFlowResponseMessage.paramsJson);
    msg.body = params.id; // Use the extracted ID as the body
  } else {
    msg.body =
      ...
      (msg.type === "buttonsResponseMessage" && msg.message[msg.type].selectedButtonId) || // Handling button response messages
      (msg.type === "templateButtonReplyMessage" && msg.message[msg.type].selectedId) || // Handling template button reply messages
      (msg.type === "interactiveResponseMessage" &&
        JSON.parse(msg.message[msg.type].nativeFlowResponseMessage.paramsJson).id) || // Handling interactive messages
...
// Log the extracted body for verification
