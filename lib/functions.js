...
function createInteractiveMessage(data, options = {}) {
  ...
  // Iterating through the buttons to structure them correctly
  for (let i = 0; i < button.length; i++) {
    let btn = button[i];
    let Button = {
      buttonParamsJson: JSON.stringify(btn.params),
      name: getButtonType(btn.type),  // Using a helper function for button type mapping
    };
    buttons.push(Button);
  }

  // Structuring the interactive message object
  const mess = {
    viewOnceMessage: {
      message: {
        messageContextInfo: {
  ...
  return mess; // Returning the message object to use later
}

// Helper function to map button types to their respective names
...
// Function to handle interactive response messages
function handleInteractiveResponse(m) {
  if (m.type === "interactiveResponseMessage") {
    try {
      const responseId = JSON.parse(m.message.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson).id;
      console.log("Interactive Response ID:", responseId);

      // You can add logic here to handle specific responses by ID
      return responseId;  // Returning the parsed ID if needed
    } catch (error) {
      console.error("Error parsing interactive response message:", error);
    }
  }
  return null;
}

// Function to handle message types and extract text/body
function handleMessageContent(m) {
  try {
    m.text = m.message.conversation ||
      (m.message[m.type] && m.message[m.type].text) ||
      (m.message[m.type] && m.message[m.type].selectedId) ||
      (m.type === "interactiveResponseMessage" && 
        JSON.parse(m.message.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson).id);

    m.body = m.message.conversation ||
      (m.message[m.type] && m.message[m.type].text) ||
      (m.message[m.type] && m.message[m.type].caption) ||
