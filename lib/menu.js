...
// Sort the button array by display_text
buttonArray2.sort((a, b) => a.params.display_text.localeCompare(b.params.display_text));

// Constructing the message object
let fak = {
    jid: message.jid,
    button: buttonArray2, // Use the sorted button array
    header: {
        title: fek,
        subtitle: fek, // You can customize this as needed
        hasMediaAttachment: true, // Set this based on the media you are sending
    },
    footer: {
        text: `𝗠𝗢𝗗𝗘 : ${type}\n𝗩𝗘𝗥𝗦𝗜𝗢𝗡 : ${version}\n${config.BOT_NAME}`,
    },
    body: {
        text: "", // You can add any text you want here
    },
};

// Assuming 'link' is defined and holds the URL of the media
if (link.endsWith(".mp4")) {
    fak.header.videoMessage = link; // Assign the video URL
} else {
    fak.header.imageMessage = link; // Assign the image URL
}

// Send the interactive message
return await message.sendMessage(message.jid, fak, {}, "interactive");     
        }
...
async function setMenuType(message, match) {
    ...
    const menuvalue = menutyp === 'text' ? 'v1' : menutyp === 'button' ? 'v2' : menutyp === 'list' ? 'v3';
